...
def get_repo_overview(request, owner: str, repo: str):
    import asyncio
    # 在同步上下文中獲取 access_token
    access_token = request.session.get('access_token')
    # 將 access_token 作為參數傳給異步函數
    result = asyncio.run(get_repo_overview_async(request, owner, repo, access_token))
    return result

@csrf_exempt
@require_GET
async def get_repo_overview_async(request, owner: str, repo: str, access_token: str):
    #access_token = request.session.get('access_token')
    if not access_token:
...
def chat_with_repo(request, owner: str, repo: str):
    import asyncio
    # 在同步上下文中獲取 access_token
    access_token = request.session.get('access_token')
    ...
    # 將 access_token 作為參數傳給異步函數
    result = asyncio.run(chat_with_repo_async(request, owner, repo, access_token, question, target_sha))
    return result

@csrf_exempt
@require_GET
async def chat_with_repo_async(request, owner: str, repo: str, access_token: str, question: str, target_sha: str):
    try:
        if not question:
            try:
...
def analyze_commit_diff(request, owner: str, repo: str, sha: str):
    import asyncio
    # 在同步上下文中獲取 access_token
    access_token = request.session.get('access_token')
    # 將 access_token 作為參數傳給異步函數
    result = asyncio.run(analyze_commit_diff_async(request, owner, repo, sha, access_token))
    return result

@csrf_exempt
@require_GET
async def analyze_commit_diff_async(request, owner: str, repo: str, sha: str, access_token:str):
    if not access_token:
