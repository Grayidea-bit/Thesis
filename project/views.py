...
def get_available_model() -> str:
    try:
        ...
        return JsonResponse({"detail": f"Failed to list Gemini models due to API error: {str(e)}"}, status=500)
    except Exception as e:
        logger.error(f"列出 Gemini 模型時發生意外錯誤: {str(e)}", exc_info=True)
        return JsonResponse({"detail": f"An unexpected error occurred while listing models: {str(e)}"}, status=500)
...
async def generate_gemini_content(model_instance: genai.GenerativeModel, prompt_text: str) -> str:
    try:
            ...
            if response.candidates:
                for candidate in response.candidates:
                    if candidate.finish_reason != 1:
                        ...
                        return JsonResponse({"detail": detail_message}, status=400)
            return JsonResponse({"detail": "Gemini API returned an empty or invalid response."}, status=500)
        ...
        return JsonResponse({"detail": f"Gemini API failed (non-quota): {str(e)}"}, status=500)
        ...
        return JsonResponse({"detail": f"Gemini API retries failed: {str(original_error)}"}, status=500)
    except Exception as e:
        logger.error(f"生成 Gemini 內容時發生意外錯誤 (模型: {model_instance.model_name}): {str(e)}", exc_info=True)
        return JsonResponse({"detail": f"Unexpected error during Gemini content generation: {str(e)}"}, status=500)
...
async def github_callback(request):
    ...
    if not code:
        return JsonResponse({"detail": "GitHub code not provided"}, status=400)
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if not access_token:
                ...
                return JsonResponse({"detail": f"無法獲取 GitHub access token: {error_description}"}, status=400)
            ...
            return JsonResponse({"detail": f"GitHub OAuth 回呼失敗: {e.response.text}"}, status=e.response.status_code)
        except Exception as e:
            logger.error(f"GitHub OAuth 回呼期間發生意外錯誤: {str(e)}", exc_info=True)
            return JsonResponse({"detail": f"GitHub OAuth 回呼期間發生意外錯誤: {str(e)}"}, status=500)
...
async def get_repos(request):
    ...
    if not access_token:
        return JsonResponse({"detail": "Missing access token"}, status=400)
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            return JsonResponse(repos_data, safe=False)
        except httpx.HTTPStatusError as e:
            ...
            return JsonResponse({"detail": f"獲取倉庫列表時發生意外錯誤: {str(e)}"}, status=500)
        ...
        async with httpx.AsyncClient() as client:
            while True:
                try:
                    ...
                    return JsonResponse({"detail": detail}, status=e.response.status_code)
...
@csrf_exempt
@require_GET
async def get_commits(request, owner: str, repo: str):
    access_token = request.GET.get("access_token")
    if not access_token:
    ...
    try:
        ...
        return JsonResponse({"detail": f"獲取 {owner}/{repo} commits 時發生意外錯誤: {str(e)}"}, status=500)

@csrf_exempt
@require_GET
async def get_commit_diff(request, owner: str, repo: str, sha: str):
    access_token = request.GET.get("access_token")
    if not access_token:
        return JsonResponse({"detail": "Missing access token"}, status=400)
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if len(diff_text) > 100000:
                return JsonResponse({"detail": "Diff too large, please select a smaller commit"}, status=400)
            return JsonResponse({"sha": sha, "diff": diff_text})
        except httpx.HTTPStatusError as e:
            logger.error(f"獲取 commit diff 失敗: {str(e)}, 狀態碼: {e.response.status_code}, 回應: {e.response.text}")
            return JsonResponse({"detail": f"Failed to fetch commit diff: {e.response.text}"}, status=e.response.status_code)

@csrf_exempt
@require_GET
async def get_repo_overview(request, owner: str, repo: str):
    access_token = request.GET.get('access_token')
    if not access_token:
    ...
    if not await validate_github_token(access_token):
        return JsonResponse({'detail': 'Invalid or expired GitHub token.'}, status=401)

    try:
        async with httpx.AsyncClient() as client:
            commit_map, commits_data = await get_commit_number_and_list(owner, repo, access_token)
            if isinstance(commit_map, JsonResponse):
                return commit_map
            if not commits_data:
                logger.info(f"倉庫 {owner}/{repo} 無 commits，無法生成概覽。")
                return JsonResponse({'detail': 'No commits found in repository, cannot generate overview.'}, status=404)
            ...
            if first_commit_number is None:
                ...
                return JsonResponse({'detail': 'Failed to determine first commit number for overview.'}, status=500)
...
說明這個倉庫的核心功能和主要目的。請明確 Technician提及這是基於「第一次 commit」的分析。
...
"""
        try:
            ...
            if isinstance(overview_text, JsonResponse):
                return overview_text
            logger.info(f"Gemini 概覽結果 (模型: {selected_model_name}): {overview_text[:150]}...")
            return JsonResponse({'overview': overview_text})
        except httpx.HTTPStatusError as e:
            ...
            return JsonResponse({'detail': f"生成倉庫概覽時發生意外錯誤: {str(e)}"}, status=500)
    except Exception as e:
        logger.error(f"chat_with_repo 意外錯誤: {str(e)}", exc_info=True)
        return JsonResponse({'detail': f"Unexpected error: {str(e)}"}, status=500)
    
@csrf_exempt
@require_POST
async def chat_with_repo(request, owner: str, repo: str):
    try:
        access_token = request.GET.get('access_token')
        question = request.POST.get('question') or request.GET.get('question')
        target_sha = request.GET.get('target_sha')
        if not question:
            try:
                body = json.loads(request.body.decode('utf-8'))
                question = body.get('question')
            except (json.JSONDecodeError, ValueError) as e:
                logger.error(f"解析 JSON body 失敗: {str(e)}")

        if not access_token or not question:
            missing = [p for p, v in [("access_token", access_token), ("question", question)] if not v]
            return JsonResponse({'detail': f"缺少必要的查詢參數: {', '.join(missing)}"}, status=400)
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if previous_commit_files_content_text:
                    ...
                    if target_sha and previous_commit_sha_for_context:
...
請提供你的回答:
"""
                log_prompt = prompt[:400] + "..." if len(prompt) > 400 else prompt
                ...
                if isinstance(answer_text, JsonResponse):
                    return answer_text
                ...
                return JsonResponse({"answer": answer_text, "history": conversation_history[history_key]})
            except httpx.HTTPStatusError as e:
                ...
                return JsonResponse({'detail': f"處理對話時發生意外錯誤: {str(e)}"}, status=500)
    except Exception as e:
        logger.error(f"chat_with_repo 意外錯誤: {str(e)}", exc_info=True)
        return JsonResponse({'detail': f"Unexpected error: {str(e)}"}, status=500)
...
@csrf_exempt
@require_POST
async def analyze_commit_diff(request, owner: str, repo: str, sha: str):
    access_token = request.GET.get('access_token')
    if not access_token:
        return JsonResponse({'detail': 'Access token is missing.'}, status=400)
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if isinstance(commit_map, JsonResponse):
                return commit_map
            if not commits_data:
                return JsonResponse({'detail': 'No commits found in repository, cannot perform analysis.'}, status=404)
            ...
            if not target_commit_obj:
                ...
                try:
                    ...
                    return JsonResponse({'detail': f"Target commit SHA {sha} not found in repository {owner}/{repo}."}, status=404)
...
"""
            ...
            return JsonResponse({'detail': f"分析 commit diff 時發生意外錯誤: {str(e)}"}, status=500)